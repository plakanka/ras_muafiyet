...
Music LoadMusicStream(const char *fileName)
{
    ...
    else
    {
        ...
        TRACELOG(LOG_INFO, "    > Channels:      %i (%s)", music.stream.channels, (music.stream.channels == 1) ? "Mono" : (music.stream.channels == 2) ? "Stereo"
                                                                                                                                                       : "Multi");
...
void UpdateMusicStream(Music music)
{
    if (music.stream.buffer == NULL)
        return;
    if (!music.stream.buffer->playing)
        return;
...
float GetMusicTimePlayed(Music music)
{
    ...
    if (music.stream.buffer != NULL)
    {
...
#endif
        {
            ...
            {
                if (!music.looping)
                    framesInBuffers = music.frameCount;
            }
...
AudioStream LoadAudioStream(unsigned int sampleRate, unsigned int sampleSize, unsigned int channels)
{
    ...
    if (deviceBitsPerSample > 4)
        deviceBitsPerSample = 4;
